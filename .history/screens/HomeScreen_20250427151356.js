...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    fetchFixtures(); // Fetch once immediately
  
    // Set interval only if selectedDate is today
    if (selectedDate === format(new Date(), 'yyyy-MM-dd')) {
      intervalId = setInterval(() => {
        fetchFixtures(); // Re-fetch fixtures every 30 seconds
      }, 30000);
    }
  
    return () => {
      if (intervalId) {
        clearInterval(intervalId); // Clear interval when leaving screen
      }
    };
  }, [selectedDate]);
