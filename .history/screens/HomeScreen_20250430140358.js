...
export default function HomeScreen() {
  ...
  const handleFollowTeam = async (teamName) => {
    ...
    try {
      // Fetch existing followed teams
      const docSnap = await getDoc(userRef);
      let currentTeams = docSnap.exists() ? docSnap.data().followedTeams || [] : [];
  
      if (!currentTeams.includes(teamName)) {
        const updatedTeams = [...currentTeams, teamName];
        await setDoc(userRef, { followedTeams: updatedTeams }, { merge: true });
        setFollowedTeams(updatedTeams); // update local state
      }
    } catch (error) {
