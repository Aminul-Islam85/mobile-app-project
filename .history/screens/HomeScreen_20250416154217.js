...
export default function HomeScreen() {
  ...
  onPress={() => {
    if (Platform.OS === 'web') {
      inputRef.current?.focus(); // <- use focus instead of click
      setTimeout(() => inputRef.current?.click(), 50); // some browsers need this too
    } else {
...
>
  ...
  <input
    ...
    style={{
      position: 'absolute',
      top: -100, // off-screen but visible
      left: -100,
      opacity: 0.01, // still barely visible to be clickable
      zIndex: -1,
    }}
  />
)}
