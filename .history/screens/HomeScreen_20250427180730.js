...
export default function HomeScreen() {
  ...
  const handleUserIconPress = () => {
    if (auth.currentUser) {
      ...
      Alert.alert(
        'Authentication',
        'Login or Register to save favorites',
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Login/Register', onPress: () => navigation.navigate('Auth') },
        ]
      );
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      {/* Calendar and User Icon for Web */}
      {Platform.OS === 'web' && (
        <View style={{ position: 'absolute', top: 10, right: 10, alignItems: 'center' }}>
          <TouchableOpacity onPress={() => datePickerRef.current.setOpen(true)}>
      ...
      {/* Calendar and User Icon for Mobile */}
      {Platform.OS !== 'web' && (
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 10, marginRight: 10 }}>
          ...
          <TouchableOpacity onPress={handleUserIconPress} style={{ marginLeft: 12 }}>
            <FontAwesome name="user" size={28} color="#000" />
          </TouchableOpacity>
        </View>
      )}
